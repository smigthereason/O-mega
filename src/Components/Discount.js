...
//           'https://api.rawg.io/api/games?key=6e2c3d10b67342d8a5dac993f10b5393&dates=2022-01-01,2023-12-31&ordering=-added&page_size=10&genres=action,adventure,shooter'
//         );
//         if (!response.ok) {
...
const Discount = () => {
  ...
  useEffect(() => {
    const fetchGames = async () => {
      try {
        const response = await fetch(
          'https://api.rawg.io/api/games?key=6e2c3d10b67342d8a5dac993f10b5393&dates=2021-01-01,2023-12-31&ordering=-added&page_size=10&genres=action,adventure,shooter'
        );
        if (!response.ok) {
          throw new Error('Network response was not ok.');
        }
  ...
  const handleMouseEnter = (trailerUrl) => {
    setIsHovered(true);
      ...
      {games.map((game) => (
        ...
        >
          <div
            ...
            onMouseEnter={() => handleMouseEnter(game.clip?.clip)}
